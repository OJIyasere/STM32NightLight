...
void ADC_Pin_Init(void){
	...
	// Configure GPIO as No Pull-down/Pull-up
	GPIOA->PUPDR &= ~(3UL << (2*1));
...
void ADC1_2_IRQHandler(void){
	...
	if ((ADC1->ISR & ADC_ISR_EOC) == ADC_ISR_EOC) {
		...
		adc_result = ADC1->DR; // Prevent Interrupt from triggering again
		if(adc_result < 500){
			turn_on_LED();
			/*sound = 1;
			if(sound == 1){
		...
		/*
		 * Code here to input Condition checking
		 * Threshold for dark v.s. Light
		 * Turn on LED and sound Buzzer for ~1 Second
		 * Using SysTick after falling below Threshold
